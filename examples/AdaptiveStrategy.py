...
class AdaptiveStrategy(DefaultStrategy):
    ...
    learn_every: int = 200
    imitation_steps: int = 1_000 # Steps for supervised imitation learning before RL
    ...
    def initialize_state(self, scene_scale: float = 1.0) -> dict[str, Any]:
        ...
        state["prune_reward_queue"] = deque(maxlen=10_000)

        # Buffers for the GROWING agent
        state["grow_replay_buffer"] = TensorDictReplayBuffer(
            storage=LazyMemmapStorage(max_size=30_000), sampler=RandomSampler(), batch_size=512
        )
        state["grow_reward_queue"] = deque(maxlen=10_000)

        state.update({"age": None, "l1_loss_map": None, "view_proj_matrix": None})
        return state

    def _initialize_learning_components(self, device: torch.device) -> None:
        """Initialize both pruning and growing agents."""
        # Pruning Agent: Action space [No-op, Prune]
        ...
        # Growing Agent: Action space [No-op, Split, Duplicate]
    ...
    def step_post_backward(
        ...
        # Initialize agents on the first step
        ...
        # Update shared state from the renderer
        state["age"] += 1
        ...
        # Process delayed rewards for RL phase
    ...
    def prune_gs(self, params: dict, optimizers: dict, state: dict, is_imitation_phase: bool) -> int:
        """Prunes Gaussians based on opacity."""
        ...
        self.prune_ac_net.train(is_imitation_phase) # train mode for SL, eval for RL sampling
        action_logits, _ = self.prune_ac_net(features)

        if is_imitation_phase:
            # Expert label: always prune if opacity is low
            ...
            # Defer reward calculation by adding to a queue (handled in _process_rewards)
    ...
    def grow_gs(self, params: dict, optimizers: dict, state: dict, is_imitation_phase: bool) -> Tuple[int, int]:
        """Grows Gaussians based on view-space gradients."""
        device = params["means"].device
        ...
        if is_imitation_phase:
            # Expert label: split large Gaussians, duplicate small ones
            scales = torch.exp(params["scales"][original_indices])
            is_large_mask = scales.max(dim=-1).values > state["scene_scale"]
            # Action 1 for Split (large), Action 2 for Duplicate (small)
            labels = torch.ones(len(original_indices), dtype=torch.long, device=device) # Default to Split
            labels[~is_large_mask] = 2 # Set to Duplicate
            experience = TensorDict({"features": features, "action_label": labels}, batch_size=[len(labels)])
    ...
    def _train_imitation_agent(self, state: dict, agent_type: str):
        """Trains an agent using supervised cross-entropy loss."""
    ...
    def _process_rewards(self, state: dict, current_step: int):
        """Processes delayed rewards for both agents."""
    ...
    def _get_simplified_features(self, params: dict, state: dict, subset_mask: Tensor) -> Tensor:
        ...
        # Cache params for feature calculation to avoid passing them everywhere
        ...
        features[:, 2] = torch.sigmoid(params["opacities"][subset_mask].flatten())

        features[:, 3] = state["age"][subset_mask] / 1000.0 # Normalized age

        if n_subset > 5:
            means3d_subset = params["means"][subset_mask]
            dists, _ = knn_with_ids(means3d_subset, K=4)
            features[:, 4] = dists[:, 1:].mean(dim=-1) / state["scene_scale"]

        gt_pixels = state.get("pixels", torch.tensor(0)).squeeze(0)
        if gt_pixels.numel() > 1:
            h, w, _ = gt_pixels.shape
            means_h = F.pad(params["means"][subset_mask], (0, 1), value=1.0)
            p_hom = means_h @ state["view_proj_matrix"]
            p_w = 1.0 / (p_hom[:, 3].clamp_min(1e-6))
            p_proj = p_hom[:, :2] * p_w[:, None]
            pixel_x = torch.clamp(((p_proj[:, 0] * 0.5 + 0.5) * w), 0, w - 1).long()
            ...
            patch_complexities = gt_pixels[pixel_y, pixel_x].mean(dim=-1).std(dim=0, keepdim=True).expand(n_subset)
